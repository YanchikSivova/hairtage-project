...
export default function AdminProductForm() {
  ...
  useEffect(() => {
    setPreviewImageSrc(getPublicImagePath(picUrl))
  }, [picUrl])
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        setPicUrl(normalizeStoredImagePath(product.picUrl || ''))
  ...
  const validateForm = () => {
    ...
    const trimmedPicUrl = normalizeStoredImagePath(picUrl)
  ...
  const handleSubmit = async (e) => {
    ...
    const productData = {
      ...
      ingredients: ingredientsInput
        ...
        .map((item) => item.trim())
        .filter(Boolean),
      picUrl: normalizeStoredImagePath(picUrl),
      price,
    }
  ...
  const handleSaveImage = (newUrl) => {
    const normalized = normalizeStoredImagePath(newUrl)

    setPicUrl(normalized)
  ...
  const handlePreviewImageError = (e) => {
    ...
    const alternateSrc = getAlternateImagePath(currentSrc)

    if (alternateSrc && alternateSrc !== currentSrc) {
      img.dataset.fallbackApplied = 'true'
      setPreviewImageSrc(alternateSrc)
      return
    }
